 ...
 * @author Jerome Cintas- Paybox Services (http://www1.paybox.com)
...
define ('OSC_ETRANSACTIONS_KEY_PATH',realpath(dirname(__FILE__))."kek.php");
...
class ETransactionsEncrypt {
	...
	public function generateKey(){
		...
		if(file_exists(OSC_ETRANSACTIONS_KEY_PATH))unlink(OSC_ETRANSACTIONS_KEY_PATH);
		$key = bin2hex($key);
		$td = mcrypt_module_open(MCRYPT_RIJNDAEL_128, '', MCRYPT_MODE_CBC, '');
		$size = mcrypt_enc_get_iv_size($td);
    	$iv = mcrypt_create_iv($size, MCRYPT_RAND);
		$iv = bin2hex($iv);
		file_put_contents(OSC_ETRANSACTIONS_KEY_PATH,"<?php" . $key.$iv ."?>");
		// Init vector
	 ...
	 private function _getKey()
	{
		...
		if(!file_exists(OSC_ETRANSACTIONS_KEY_PATH)){
			$this->generateKey();
			$_POST['KEY_ERROR'] = "For some reason, the key has just been generated. please reenter the HMAC key to crypt it.";
		}else{
			$key = file_get_contents(OSC_ETRANSACTIONS_KEY_PATH);
			$key = substr($key,5,32);
			return $key;
		}
	}
	 ...
	 private function _getIv()
	{
		...
		if(!file_exists(OSC_ETRANSACTIONS_KEY_PATH)){
			...
			$iv = file_get_contents(OSC_ETRANSACTIONS_KEY_PATH);
			$iv = substr($iv,37,16);
			return $iv;
		}
	}
